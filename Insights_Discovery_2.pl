...
inventor :- %INTJ BLUE
verify(work_environment_has_formal_professional_layout),
verify(work_environment_is_organised_and_with_information_available_at_arms_length),
verify(stressed_by_lack_of_information_structure__and_logic),
verify(stressed_by_poor_quality_work),
verify(stressed_by_time_wasted_or_task_rushed),
verify(may_become_questioning_and_deliberate_when_stressed),
verify(may_tend_to_nitpick_when_stressed),
verify(may_tend_to_be_aloof_withdrawn_and_resentful_when_stressed),
...
healer :- %INFJ GREEN
verify(work_environment_has_a_warm_and_friendly_vibe),
verify(work_environment_usually_has_family_photographs),
verify(work_environment_usually_has_small_plants),
verify(stressed_by_unfair_or_impersonal_treatment),
verify(stressed_by_violation_of_values),
verify(stressed_by_interruptions_or_time_pressures),
verify(may_become_silent_withdrawn_or_hurt_when_stressed),
verify(may_tend_to_be_judgemental_impersonal_resistant_when_stressed),
verify(may_tend_to_be_stubborn_and_overly_cautious_when_stressed),
verify(feel_like_everything_can_and_should_be_made_better),
verify(tend_to_use_metaphors_analogies_or_theoretical_language_when_describing_things),
verify(tend_to_be_slow_in_replying_or_making_decisions), %prefer to have a lot of time to analyze information
verify(daydreams),
...
challenger :- %ESTP RED
verify(work_environment_has_orderly_businesslike_layout),
verify(work_environment_has_the_correct_tools_for_the_job),
verify(work_environment_usually_has_time_saving_gadgets_planners_or_personal_organizers),
verify(stressed_by_lack_of_focus),
verify(stressed_by_indecisiveness),
verify(stressed_by_being_out_of_control),
verify(may_become_aggressive_when_stressed),
verify(may_tend_to_be_impatient_when_stressed),
verify(may_tend_to_be_irritable_and_demanding_when_stressed),
verify(flexible),
verify(easily_get_bored_if_life_is_monotonous),
verify(tend_to_use_non_verbal_communication_when_speaking), %i.e. hand gestures, winks, facial expressions
...
king :- %ESFP YELLOW
verify(work_environment_has_unstructured_layout),
verify(work_environment_usually_has_group_photos),
verify(work_environment_usually_has_plaques_trophies_etc),
verify(stressed_by_restriction_on_flexibility),
verify(stressed_by_no_interaction_and_fun),
verify(stressed_by_personal_rejection),
verify(may_become_over_responsive_when_stressed),
verify(may_tend_to_appear_opinionated_when_stressed),
verify(may_tend_to_be_argumentative_when_stressed),
verify(flexible),
verify(easily_get_bored_if_life_is_monotonous),
verify(tend_to_use_non_verbal_communication_when_speaking), %i.e. hand gestures, winks, facial expressions
...
/* ASKING QUESTIONS */
ask(Question) :-
write('Traits/Habits/Mannerisms: '),
write(Question), nl,
write('> '),
read(Response),
nl,
